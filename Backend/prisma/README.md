...
wsl -d Ubuntu -- sudo -u postgres psql -c "CREATE ROLE craviox LOGIN PASSWORD 'craviox' SUPERUSER;"
wsl -d Ubuntu -- sudo -u postgres createdb -O craviox craviox
wsl -d Ubuntu -- sudo -u postgres psql -d craviox -c 'CREATE EXTENSION postgis;'
```
...
```
DATABASE_URL="postgresql://craviox:craviox@localhost:5434/craviox?schema=public"
```
...
```bash
docker run -d --name craviox-pg \
  -e POSTGRES_USER=craviox -e POSTGRES_PASSWORD=craviox -e POSTGRES_DB=craviox \
...
```
DATABASE_URL="postgresql://craviox:craviox@localhost:5433/craviox?schema=public"
```

### Planner tuning

```sql
ALTER DATABASE craviox SET random_page_cost = 1.1;
```
