# Go live on craviox.com

Target host: EC2 `craviox-server` (`i-0da155967af299582`), `3.110.207.151`,
ap-south-1. Ubuntu 26.04, 1 vCPU, 2 GB RAM, 8 GB EBS.

| Host                      | Serves                    | Backed by                              |
| ------------------------- | ------------------------- | -------------------------------------- |
| `craviox.com`       | prod API + socket + files | PM2 `craviox-*`, ports 5000/5001    |
| `craviox.com`     | prod web app (SPA)        | static build in `/srv/craviox/admin`     |
| `uat.craviox.com`   | UAT API + socket + files  | PM2 `uat-craviox-*`, ports 5100/5101     |
| `uat.craviox.com` | UAT web app (SPA)         | static build in `/srv/craviox-uat/admin` |
...
- Frontend built and deployed to `/srv/craviox/admin`, with
  `https://craviox.com/api/v1` baked in.
...
- Rebranded to Craviox; `craviox.com/` lands on `/admin`.
...
## 1. DNS (client, in Cloudflare)
    ...
    craviox.com          A   3.110.207.151
    craviox.com        A   3.110.207.151
    uat.craviox.com      A   3.110.207.151
    uat.craviox.com    A   3.110.207.151

Confirm before running certbot — it fails on a name that has not propagated:

    dig +short craviox.com
...
Port 5432 is reachable from the EC2 instance.
    ...
    DATABASE_URL=postgresql://USER:PASS@HOST:5432/craviox?schema=public&sslmode=require
...
same build, so both call the same API:
    ...
    CORS_ORIGINS=https://craviox.com,https://www.craviox.com,https://craviox.com,https://uat.craviox.com
...
Media goes to the `craviox-media` bucket in ap-south-1, written by the EC2
instance role -- no access keys anywhere. Set in `Backend/.env`:

    UPLOAD_DRIVER=s3
    UPLOAD_S3_BUCKET=craviox-media
    UPLOAD_S3_REGION=ap-south-1
    UPLOAD_BASE_URL=https://craviox-media.s3.ap-south-1.amazonaws.com
...
granting `s3:GetObject` on `arn:aws:s3:::craviox-media/*` is what actually
grants the read. Neither works alone.

Worth doing later: CloudFront in front of the bucket, on `cdn.craviox.com`.
...
## 6. Verify
    ...
    curl -sS https://craviox.com/health

    # CORS: header present on the first, absent on the second
    curl -sSI -H 'Origin: https://craviox.com' https://craviox.com/api/v1/health
    curl -sSI -H 'Origin: https://evil.example.com'    https://craviox.com/api/v1/health

Then at `https://craviox.com`: load `/admin`, sign in, open an order.
The console should show a `/socket.io/` upgrade and no CORS or mixed-content
errors, and images should load from `craviox.com/uploads/`.
